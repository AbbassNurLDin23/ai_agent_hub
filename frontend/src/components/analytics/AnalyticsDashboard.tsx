...
import type { MetricsStreamData, Agent } from "@/types";
...
export function AnalyticsDashboard({
  ...
  // ✅ backend returns camelCase in current:
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={formatNumber(current.messagesCount)}
          icon={<MessageSquare className="w-6 h-6 text-primary" />}
          trendLabel="All time"
        />
        <StatCard
          title="Tokens Processed"
          value={formatNumber(current.tokensProcessed)}
          icon={<Zap className="w-6 h-6 text-primary" />}
          trendLabel="All time"
        />
        <StatCard
          title="Avg Latency"
          value={`${Math.round(current.avgLatencyMs)}ms`}
          icon={<Clock className="w-6 h-6 text-primary" />}
          trendLabel="Rolling average"
        />
        <StatCard
          title="Last Response"
          value={`${Math.round(current.lastResponseLatencyMs)}ms`}
